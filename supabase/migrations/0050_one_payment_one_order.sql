-- ============================================================
-- 0050 — one payment can no longer become two orders
--
-- THE BUG, which is live right now.
--
-- place_order reads the cart, writes the order, then deletes the cart, all
-- in one transaction. A second tap a moment later finds an empty cart and
-- stops, which is why this was never noticed. But two SIMULTANEOUS calls —
-- a double-tap, or a phone retrying on a bad connection — both read the cart
-- before either one commits. Both pass the "cart is empty" check, both write
-- an order, both delete the same rows. The customer is charged twice for one
-- payment, and once gift cards are minted at checkout it would mint two
-- cards for one payment too.
--
-- Disabling the button while the request is in flight does NOT fix this. The
-- browser is not the boundary; a retry can come from the network stack, or
-- from a second device.
--
-- THE FIX
--
-- One statement, at the very top: take a row lock on this shopper's cart
-- lines before reading anything. The second transaction then blocks until
-- the first commits, wakes to find no rows left, and stops on the "cart is
-- empty" check that already exists. No new error path and no new state — the
-- second tap simply behaves exactly like tapping checkout on an empty cart.
--
-- Everything else in this file is byte-for-byte the function currently in
-- production (0047). It was extracted from that migration rather than
-- retyped, so the lock is the only difference.
-- ============================================================

drop function if exists place_order(uuid, date, text, text, text, text, boolean, text, text, text, boolean, text, uuid);
