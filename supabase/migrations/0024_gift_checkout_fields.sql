-- PROMPT 8 (spec 4.6): the gift-first checkout.
--
-- Two things the current schema couldn't express:
--   1. Who the gift is actually for — orders only knew the buyer.
--   2. That the address may not be known yet, because the whole point of
--      the WhatsApp flow is that you rarely know your friend's address.
--
-- Both are additive. delivery_address_id becomes nullable so an order can
-- exist while we're still collecting the address from the recipient.

alter table orders
  add column if not exists is_gift boolean not null default true,
  add column if not exists recipient_name text,
  add column if not exists recipient_phone text,
  -- 'buyer'  = buyer typed the address
  -- 'recipient_whatsapp' = we ask the recipient for it via WhatsApp
  add column if not exists address_source text not null default 'buyer'
    check (address_source in ('buyer', 'recipient_whatsapp')),
...
alter table orders
  add constraint orders_recipient_name_len check (recipient_name is null or char_length(recipient_name) <= 200),
  add constraint orders_recipient_phone_len check (recipient_phone is null or char_length(recipient_phone) <= 30),
  add constraint orders_gift_message_len check (gift_message is null or char_length(gift_message) <= 500),
  add constraint orders_delivery_slot_len check (delivery_slot is null or char_length(delivery_slot) <= 100);

-- An order must end up deliverable: either we have an address, or we have a
-- phone number to go and ask for one.
alter table orders add constraint orders_deliverable check (
  delivery_address_id is not null
  or (address_source = 'recipient_whatsapp' and recipient_phone is not null)
);
...
revoke all on function place_order(uuid, date, text, text, text, text, boolean, text, text, text, boolean, text) from public, anon;
grant execute on function place_order(uuid, date, text, text, text, text, boolean, text, text, text, boolean, text) to authenticated;
